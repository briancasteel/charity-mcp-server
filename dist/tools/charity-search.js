...
import { CharitySearchInputSchema } from "../schemas/charity-schemas.js";
...
export async function handleCharitySearch(args) {
    try {
...
function formatCharitySearchResponse(output, input) {
    const { results, pagination } = output;
    let response = `# Charity Search Results\n\n`;
    // Search summary
    const searchCriteria = [];
    if (input.query)
        searchCriteria.push(`Query: "${input.query}"`);
    if (input.city)
        searchCriteria.push(`City: ${input.city}`);
    if (input.state)
        searchCriteria.push(`State: ${input.state}`);
...
//# sourceMappingURL=charity-search.js.map